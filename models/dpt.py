...
def get_depth_map(image, token=None):
    try:
        ...
        model = AutoModelForDepthEstimation.from_pretrained("LiheYoung/depth-anything-small-hf", use_auth_token=token).to("cpu")

        print("Preparing image for the model...")
        encoding = image_processor(images=image, return_tensors="pt").to("cpu")
...
# Load an image
image_path = "D:\Height-Detection-main\Height-Detection-main\images\height.png"  # Replace with the path to your image
